...
export default function ProductDetailClient({ slug }: { slug: string }) {
  ...
  if (error) return (
    <div className="mx-auto max-w-7xl px-4 py-12">
      <p role="alert" className="text-red-600 dark:text-red-400">Unable to load this product. Please try again.</p>
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
      <Link href="/product" className="inline-flex h-11 items-center gap-2 rounded-full border border-slate-300 px-5 text-sm font-semibold text-slate-600 hover:bg-slate-100 hover:text-slate-950 dark:border-slate-600 dark:text-slate-400 dark:hover:bg-slate-800 dark:hover:text-white"><ArrowLeft className="size-4" /> Back to products</Link>
      <div className="mt-8 grid overflow-hidden rounded-[2rem] border border-slate-200 bg-white shadow-sm lg:grid-cols-2 dark:border-slate-700 dark:bg-slate-900">
        <div className="flex min-h-[420px] items-center justify-center bg-slate-100 p-10 dark:bg-slate-800">
          <Image src={product.image} alt={product.title} width={500} height={500} className="max-h-96 w-full object-contain" />
        </div>
        <div className="p-8 sm:p-12">
          <p className="text-sm font-bold uppercase tracking-[0.2em] text-amber-600 dark:text-amber-400">Product</p>
          ...
          <p className="mt-2 text-lg leading-8 text-slate-600 dark:text-slate-400">{product.description}</p>
          <div className="mt-7 space-y-3 text-sm text-slate-600 dark:text-slate-400">
